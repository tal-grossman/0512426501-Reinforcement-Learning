...
def compute_vpi(pi, mdp, gamma):
    ...
    prev_V = np.zeros(mdp.nS)
    for i in range(150):
        Vi = np.zeros(mdp.nS)
        for state in range(mdp.nS):
            action = pi[state]
            for prob, next_state, reward in mdp.P[state][action]:
                Vi[state] += prob * (reward + gamma * prev_V[next_state])
        prev_V = Vi
    return Vi
...
Vs_PI = np.array(Vs_PI)
plt.figure()
for state in range(mdp.nS):
    Vs = Vs_PI[:,state]
    plt.plot(Vs, 'o-', label='state %i'%state)
plt.xlabel("Iteration")
plt.ylabel("Value")
plt.title('Policy Iteration\nValue function vs interation for each state')
plt.legend()
plt.show()
